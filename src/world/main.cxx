...
int main(void){
    ...
    camera->init(vec3(0,0,-4),vec3(0,0,0),vec3(0,1,0)); //location, looking-at, up
    camera->setFOV(90.0);
    ...
    Cube * cubes[ncubes];
    for(i=0; i<ncubes; i++){
        cubes[i] = new Cube(cubePos[i], quaternion(), 0, strdup("kyubey"), TYPE1, SPAWNED, 0.1f);
        
    }

    //create skybox
    Cube *skybox = new Cube(vec3(0,0,0), quaternion(), 0, strdup("sky"), TYPE1, SPAWNED, 0.1f);
