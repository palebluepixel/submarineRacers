...
struct Material {
    ...
    color3f	ambient;	//!< ambient component
    color3f	diffuse;	//!< diffuse component
    color3f	specular;	//!< specular component
...
struct Group {
    ...
    vec3f	*verts;		//!< array of nVerts texture coordinates (or nullptr)
    vec3f	*norms;		//!< array of nVerts texture coordinates (or nullptr)
    vec2f	*txtCoords;	//!< array of nVerts texture coordinates (or nullptr)
...
class Model {
  ...
  //! the model's axis-aligned bounding box
    const AABBf &BBox () const { return this->_bbox; }
  ...
  private:
    ...
    AABBf	_bbox;		//!< bounding box for model
