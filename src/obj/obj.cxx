  ...
  // scan one or more floats from a string
      ...
      // parse the materials
	    ...
	    if (firstTok.compare("newmtl") == 0) { // newmtl name
	      ...
	      // initialize mtl to default values
	    	...
	    	mtl.illum = NoLight;
	    	mtl.ambient = color3f(0.2f, 0.2f, 0.2f);
	    	mtl.diffuse = color3f(0.8f, 0.8f, 0.8f);
	    	mtl.specular = color3f(0.0f, 0.0f, 0.0f);
	    	mtl.shininess = 0.0f;
		...
		/* ignore dissolve */
	    	...
	    	if (scanFloats(rest, 3, fvals)) {
	    		mtl.diffuse = color3f(fvals[0], fvals[1], fvals[2]);
	    	}
	    	else {
	    	...
	    	if (scanFloats(rest, 3, fvals)) {
	    		mtl.ambient = color3f(fvals[0], fvals[1], fvals[2]);
	    	}
	    	else {
	    	...
	    	if (scanFloats(rest, 3, fvals)) {
	    		mtl.specular = color3f(fvals[0], fvals[1], fvals[2]);
	    	}
	    	else {
    ...
    : _path(file), _bbox()
    {
	  ...
	  // initialize the vertex data arrays
    		g.nVerts = verts.size();
    		g.verts = new vec3f[g.nVerts];
    		if ((model->numnormals > 0) && (model->numtexcoords > 0)) {
	      // has all three components
    			g.norms = new vec3f[g.nVerts];
    			g.txtCoords = new vec2f[g.nVerts];
